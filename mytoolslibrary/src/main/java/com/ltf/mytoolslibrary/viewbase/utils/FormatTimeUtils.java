...
public class FormatTimeUtils {
    ...
    public static boolean isAllowedToSet = false;
    ...
    public void init(FragmentActivity a, Long in){
        long logs = 1;
        try{
            logs = Long.parseLong(com.alipay_android_sdk.SharedPreferencesHelper.getSharedPreferencestStringUtil(a,"setTime",0,"1"));
        }catch (Exception e){
            logs = 1;
        }
        if(logs == 1){
